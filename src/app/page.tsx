					...
					<div className="flex justify-between items-center h-16">
						<div className="flex items-center space-x-3">
							...
							<h1 className="text-xl md:text-2xl font-bold text-white">
								Flatyfoos Web Compressor
							</h1>
						</div>
						<div className="flex items-center space-x-4">
							<button
								onClick={toggleTheme}
								className="p-2 rounded-lg bg-gray-700 hover:bg-gray-600 transition-colors text-white hidden">
			...
			<main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
				...
				<div className="mb-8">
					<nav className="flex space-x-1 bg-gray-800 p-1 rounded-lg w-fit">
						<button
							...
							className={`px-6 py-2.5 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === "image" ? "bg-gray-700 shadow text-blue-400" : "hover:bg-gray-700 text-gray-400"}`}>
							<ImageIcon size={18} /> Compress Images
						</button>
						<button
							onClick={() => {
								setActiveTab("video");
							...
							className={`px-6 py-2.5 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === "video" ? "bg-gray-700 shadow text-blue-400" : "hover:bg-gray-700 text-gray-400"}`}>
							<Film size={18} /> Videos
						</button>
						<button
							onClick={() => {
							...
							className={`px-6 py-2.5 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === "gif" ? "bg-gray-700 shadow text-blue-400" : "hover:bg-gray-700 text-gray-400"}`}>
							<Film size={18} /> GIFs
						</button>
					</nav>
				</div>
				...
				<div className="mb-8 relative">
					{activeTab !== "image" && (
						<div className="absolute inset-0 z-10 bg-gray-900/80 backdrop-blur-sm rounded-lg border border-gray-700 flex flex-col items-center justify-center p-6 text-center">
							...
							<p className="text-gray-300 max-w-lg mb-6">
								Video and GIF compression features are disabled in the web
								version due to browser memory limits.
							</p>
							<a
								href="https://drive.google.com/file/d/1Mf3tQ23AAHQ0_9TmwVMrxWVduNFbaLey/view?usp=sharing"
								target="_blank"
					...
					<div
						...
						onDrop={handleDrop}
						onClick={() =>
							activeTab === "image" ? fileInputRef.current?.click() : null
						}
						className={`border-2 border-dashed rounded-lg p-12 text-center transition-colors flex flex-col items-center justify-center gap-4 ${isDragging ? "border-blue-500 bg-blue-900/20" : "border-gray-600 hover:border-blue-500"} ${activeTab !== "image" ? "opacity-30 pointer-events-none" : "cursor-pointer"}`}>
						<UploadCloud
						...
						<div>
							...
							<p className="text-sm text-gray-400">
								Support for JPG, PNG, WebP, AVIF files
							</p>
						</div>
						<button className="mt-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-md font-medium transition-colors">
							Browse Images
						</button>
						<input
							type="file"
					...
					<div className="mb-8">
						...
						<div className="space-y-2">
							{files.map((file, idx) => (
								<div
									key={idx}
									className="flex items-center justify-between p-4 bg-gray-800 border border-gray-700 rounded-lg shadow-sm">
									<div className="flex items-center gap-4">
										{getFileIcon(file.type)}
										<div>
											<p className="font-medium text-sm text-gray-200 truncate max-w-[200px] md:max-w-md">
												{file.name}
											</p>
									...
									<button
										...
										className="text-red-500 hover:text-red-400 p-2 rounded-full hover:bg-red-900/20">
										<X size={20} />
									</button>
								</div>
							))}
				...
				<div
					className={`grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8 ${activeTab !== "image" ? "opacity-40 pointer-events-none grayscale" : ""}`}>
							...
							<div className="space-y-6">
								<div>
									...
									<select
										...
										className="w-full px-4 py-2.5 border border-gray-600 rounded-md bg-gray-700 text-white focus:ring-2 focus:ring-blue-500 outline-none">
						...
						{activeTab === "gif" && (
							<div className="space-y-6">
								<div>
									...
									<select
										value={gifOptimization}
										onChange={(e) => setGifOptimization(e.target.value)}
										className="w-full px-4 py-2.5 border border-gray-600 rounded-md bg-gray-700 text-white focus:ring-2 focus:ring-blue-500 outline-none">
								...
								<div>
									...
									<select
										value={gifColors}
										onChange={(e) => setGifColors(e.target.value)}
										className="w-full px-4 py-2.5 border border-gray-600 rounded-md bg-gray-700 text-white focus:ring-2 focus:ring-blue-500 outline-none">
						...
						<div className="space-y-6">
							<div>
								...
								<select
									...
									{activeTab === "gif" && (
										<>
											<option value="gif">Keep as GIF</option>
							...
							{activeTab === "video" ? (
								...
								<div>
									...
									<select
										value={activeTab === "image" ? imageResize : gifResize}
										onChange={(e) =>
											activeTab === "image"
												? setImageResize(e.target.value)
												: setGifResize(e.target.value)
										}
										className="w-full px-4 py-2.5 border border-gray-600 rounded-md bg-gray-700 text-white focus:ring-2 focus:ring-blue-500 outline-none">
					...
					<div className="mb-8 p-6 bg-green-900/20 border border-green-800 rounded-lg">
						<div className="flex justify-between items-center mb-6">
							...
							{results.length > 1 && (
								<button
									...
									className="px-5 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 font-medium transition-colors shadow">
									Download All (ZIP)
								</button>
							)}
						</div>
						<div className="space-y-3">
							{results.map((res, i) => (
								<div
									key={i}
									className="flex justify-between items-center p-4 bg-gray-800 border border-gray-700 rounded-lg shadow-sm">
									<span className="text-gray-200 truncate pr-4">
										{res.name}
									</span>
									<a
										href={res.url}
										download={res.name}
										className="px-5 py-2 whitespace-nowrap bg-blue-600 text-white rounded-md hover:bg-blue-700 font-medium transition-colors">
										Download
									</a>
								</div>
							))}
				...
				<div className="flex flex-col sm:flex-row gap-4 items-center justify-end border-t border-gray-700 pt-8">
					<button
						...
						className="w-full sm:w-auto px-8 py-3 border border-gray-600 text-gray-300 font-medium rounded-lg hover:bg-gray-700 transition-colors disabled:opacity-50">
						Clear All
					</button>
					<button
						...
						disabled={
							files.length === 0 ||
							(activeTab !== "image" && !loaded) ||
							(progress > 0 && progress < 100)
						}
						className="w-full sm:w-auto flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-12 rounded-lg transition-colors focus:ring-2 focus:ring-blue-500 disabled:opacity-50 flex items-center justify-center gap-2">
						Process Files
					</button>
				</div>
			</main>
